...
<script>
		...
		$(document).ready(function() {
			$treeTreeTable=$('#treeTreeTable').treeTable({  
		    	   theme:'vsStyle',	           
					...
					column:0,
					checkbox: false,
		            url:'${ctx}/tree/tree/getChildren?parentId=',  
		...
		function del(con,id){  
			jp.confirm('确认要删除树的测试吗？', function(){
				jp.loading();
	       	  	$.get("${ctx}/tree/tree/delete?id="+id, function(data){
	       	  		if(data.success){
...
<script type="text/html" id="treeTreeTableTpl">
			<td>
			<c:choose>
			      <c:when test="${fns:hasPermission('tree:tree:edit')}">
				    <a  href="#" onclick="jp.openSaveDialog('编辑树的测试', '${ctx}/tree/tree/form?id={{d.row.id}}','800px', '500px')">
							{{d.row.remarks === undefined ? "": d.row.remarks}}
					</a>
			      </c:when>
			      <c:when test="${fns:hasPermission('tree:tree:view')}">
				    <a  href="#" onclick="jp.openViewDialog('查看树的测试', '${ctx}/tree/tree/form?id={{d.row.id}}','800px', '500px')">
							{{d.row.remarks === undefined ? "": d.row.remarks}}
					</a>
			      </c:when>
			      <c:otherwise>
			...
			<td>
				<div class="btn-group">
				  ...
				  <ul class="dropdown-menu" role="menu">
					<shiro:hasPermission name="tree:tree:view">
						<li><a href="#" onclick="jp.openViewDialog('查看树的测试', '${ctx}/tree/tree/form?id={{d.row.id}}','800px', '500px')"><i class="fa fa-search-plus"></i> 查看</a></li>
					</shiro:hasPermission>
					<shiro:hasPermission name="tree:tree:edit">
						<li><a href="#" onclick="jp.openSaveDialog('修改树的测试', '${ctx}/tree/tree/form?id={{d.row.id}}','800px', '500px')"><i class="fa fa-edit"></i> 修改</a></li>
		   			</shiro:hasPermission>
		   			<shiro:hasPermission name="tree:tree:del">
		   				<li><a  onclick="return del(this, '{{d.row.id}}')"><i class="fa fa-trash"></i> 删除</a></li>
					</shiro:hasPermission>
		   			<shiro:hasPermission name="tree:tree:add">
						<li><a href="#" onclick="jp.openSaveDialog('添加下级树的测试', '${ctx}/tree/tree/form?parent.id={{d.row.id}}','800px', '500px')"><i class="fa fa-plus"></i> 添加下级树的测试</a></li>
					</shiro:hasPermission>
				  </ul>
				</div>
			</td>
