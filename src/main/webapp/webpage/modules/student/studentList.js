...
$(document).ready(function() {
	$('#studentTable').bootstrapTable({
		  ...
		  //请求方法
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
				   ...
				   <c:choose>
					   <c:when test="${fns:hasPermission('student:student:edit')}">
					      return "<a href='javascript:edit(\""+row.id+"\")'>"+value+"</a>";
				      </c:when>
					  <c:when test="${fns:hasPermission('student:student:view')}">
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  jp.downloadFile('${ctx}/student/student/import/template');
				  },
			    btn2: function(index, layero){
				        var iframeWin = layero.find('iframe')[0]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
						iframeWin.contentWindow.importExcel('${ctx}/student/student/import', function (data) {
							if(data.success){
								jp.success(data.msg);
								refresh();
							}else{
	 ...
	 $("#export").click(function(){//导出Excel文件
			jp.downloadFile('${ctx}/student/student/export');
	  });
