...
<script>
		...
		$(document).ready(function() {
			$categoryTreeTable=$('#categoryTreeTable').treeTable({  
		    	   theme:'vsStyle',	           
					...
					column:0,
					checkbox: false,
		            url:'${ctx}/category/category/category/getChildren?parentId=',  
		...
		function del(con,id){  
			jp.confirm('确认要删除类别吗？', function(){
				jp.loading();
	       	  	$.get("${ctx}/category/category/category/delete?id="+id, function(data){
	       	  		if(data.success){
...
<script type="text/html" id="categoryTreeTableTpl">
			<td>
			<c:choose>
			      <c:when test="${fns:hasPermission('category:category:category:edit')}">
				    <a  href="#" onclick="jp.openSaveDialog('编辑类别', '${ctx}/category/category/category/form?id={{d.row.id}}','800px', '500px')">
							{{d.row.remarks === undefined ? "": d.row.remarks}}
					</a>
			      </c:when>
			      <c:when test="${fns:hasPermission('category:category:category:view')}">
				    <a  href="#" onclick="jp.openViewDialog('查看类别', '${ctx}/category/category/category/form?id={{d.row.id}}','800px', '500px')">
							{{d.row.remarks === undefined ? "": d.row.remarks}}
					</a>
			      </c:when>
			      <c:otherwise>
			...
			<td>
				<div class="btn-group">
				  ...
				  <ul class="dropdown-menu" role="menu">
					<shiro:hasPermission name="category:category:category:view">
						<li><a href="#" onclick="jp.openViewDialog('查看类别', '${ctx}/category/category/category/form?id={{d.row.id}}','800px', '500px')"><i class="fa fa-search-plus"></i> 查看</a></li>
					</shiro:hasPermission>
					<shiro:hasPermission name="category:category:category:edit">
						<li><a href="#" onclick="jp.openSaveDialog('修改类别', '${ctx}/category/category/category/form?id={{d.row.id}}','800px', '500px')"><i class="fa fa-edit"></i> 修改</a></li>
		   			</shiro:hasPermission>
		   			<shiro:hasPermission name="category:category:category:del">
		   			...
		   			<shiro:hasPermission name="category:category:category:add">
						<li><a href="#" onclick="jp.openSaveDialog('添加下级类别', '${ctx}/category/category/category/form?parent.id={{d.row.id}}','800px', '500px')"><i class="fa fa-plus"></i> 添加下级类别</a></li>
					</shiro:hasPermission>
				  </ul>
				</div>
			</td>
