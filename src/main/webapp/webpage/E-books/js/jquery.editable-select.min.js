(function (a) {
    ...
    a.fn.editableSelect = function (e) {
        var g = {bg_iframe: false, onSelect: false, items_then_scroll: 10, case_sensitive: false};
        var f = a.extend(g, e);
        var d = false;
    ...
    c.prototype = {
        ...
        init: function (d, f) {
            ...
            this.select = a(d);
            var e = this.select.attr("name");
            if (!e) {
                e = "editable-select" + b.length
            }
            var g = this.select.attr("id");
            if (!g) {
                g = "editable-select" + b.length
            }
            this.text = a('<input type="text">');
            this.text_submit = a('<input type="hidden">');
            this.text.attr("name", e + "_sele");
            this.text_submit.attr("name", e);
            ...
            this.text.attr("id", g + "_sele");
            this.text_submit.attr("id", g);
            this.wrapper.attr("id", g + "_editable-select-options");
            this.text.attr("autocomplete", "off");
            this.text.attr("autocomplete", "off");
            this.text.addClass("editable-select");
            this.text_submit.addClass("editable-select");
            this.select.attr("id", g + "_hidden_select");
            this.select.attr("name", e + "_hidden_select");
        ...
        duplicateOptions: function () {
            ...
            e.each(function (k) {
                ...
                var j = a('<li value="' + g + '">' + h + "</li>");
                j.hide();
                f.initListItemEvents(j);
                d.append(j)
            });
            this.setWidths();
            this.checkScroll()
        },
        in_array: function (f, d) {
        ...
        trim: function (d) {
            return typeof d == "string" ? d.replace(/^\s*|\s*$/g, "") : d
        },
        ...
        addOption: function (f, g) {
            var d = a('<li value="' + f + '">' + g + "</li>");
            var e = a('<option value="' + f + '">' + g + "</option>");
        ...
        initInputEvents: function (e) {
            ...
            e.blur(function (j) {
                var k = d.trim(this.value);
                var g = d.in_array(k, d.dataList);
                if (k == "") {
                    d.text_submit.val("")
                } else {
                    if (k != "" && !g) {
                ...
                var h = typeof d.settings.onSelect == "function" && g ? d.findItem(k) : null;
                if (typeof d.settings.onSelect == "function" && h != null) {
                    d.text.val(h.text());
                    d.text_submit.val(h.attr("value"));
                    d.current_value = d.text.val();
                    d.settings.onSelect.call(d, h, d.text_submit[0])
                }
                d.hideList();
                j.preventDefault();
                j.stopPropagation()
            ...
            e.focus(function (g) {
                ...
                (g.keyCode == 13) ? d.hideList() : d.showList();
                g.stopPropagation()
            }).keypress(function (g) {
                if (g.keyCode == 13) {
                    g.preventDefault();
        ...
        selectNewListItem: function (f) {
            ...
            if (f == "down") {
                var e = this.selectNextItem(d)
            } else {
                var e = this.selectPrevItem(d)
            }
            if (e.length) {
        ...
        selectNextItem: function (d) {
            var f = d.next();
            if (f && f[0].display == "none") {
                return d
            }
            return f
        },
        selectPrevItem: function (d) {
            var f = d.prev();
            if (f && f[0].display == "none") {
                return d
            }
        ...
        pickListItem: function (d) {
            if (d.length) {
                ...
                this.text_submit.val(d.attr("value"));
                this.current_value = this.text.val()
            }
            if (typeof this.settings.onSelect == "function") {
                this.settings.onSelect.call(this, d, this.text_submit[0])
            }
            this.hideList()
        },
        ...
        findItem: function (j) {
            var h = this;
            var f = h.trim(j);
            var d = h.wrapper.find("li");
            var g = false;
            var e = false;
            d.each(function () {
                var k = h.trim(a(this).text());
                if (!e) {
        ...
        highlightSelected: function () {
            ...
            d.each(function () {
                ...
                if (h.settings.isFilter && j.search(f) > -1 && f != "") {
                    a(this).show()
                } else {
                    if (h.settings.isFilter) {
                        a(this).hide()
                    }
                }
            });
        ...
        positionElements: function () {
            ...
            d = {top: d.top, left: d.left};
            d.top += this.text[0].offsetHeight;
            this.wrapper.css({top: d.top + "px", left: d.left + "px"});
        ...
        setWidths: function () {
            this.select.show();
            var d = this.select.width() + 2 + 20;
            this.select.hide();
            var e = parseInt(this.text.css("padding-right").replace(/px/, ""), 10);
            this.text.width(d - e + 18);
            this.wrapper.width(d + 2 + 20);
            if (this.bg_iframe) {
                this.bg_iframe.width(d + 4 + 20)
            }
        },
        createBackgroundIframe: function () {
            var d = a('<iframe frameborder="0" class="editable-select-iframe" src="about:blank;"></iframe>');
            a(document.body).append(d);
            d.width(this.select.width() + 2);
            d.height(this.wrapper.height());
            d.css({top: this.wrapper.css("top"), left: this.wrapper.css("left")});
            this.bg_iframe = d
        }
    }
});
